...
void RecognitionNLP::myTest() {
	...
	for (int c = 1; c < 10000; c *= 2) {
		CSVM::cmdRun("echo " + pathTest + "svm-train.exe -s %svm% -t %kernel% -c " + itos(c) + " -q " + pathTest + test1Training + "00.model >> " + GS_Linear);
	...
	for (int c = 1; c < 10000; c *= 2) {
		for (double g = 0.001; g < 10; g *= 2) {
			CSVM::cmdRun("echo " + pathTest + "svm-train.exe -s %svm% -t %kernel% -c " + itos(c) + " -g " + itos(g) + " -q " + pathTest + test1Training + "02.model >> " + GS_RBF);
