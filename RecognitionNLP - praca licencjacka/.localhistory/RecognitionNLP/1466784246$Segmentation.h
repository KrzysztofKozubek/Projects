		...
		for each (CImageSign var in vNLP) {
			...
			for (int i = 0; i < tmp.size() && i < var.getSign().size(); i++) {
				...
				if (sign != -1) {
					...
					vTrainingData.push_back(*countMoment(dst));
				}
...
private:
	...
	void addAngle(Mat& image) {

		CDistorted::noiseAffineTransformations(tmp[i], sizePadding, sizePadding, sizePadding, sizePadding, sizePadding, sizePadding, 0, 0, 0, 0, 0, 0);
