...
void RecognitionNLP::myTest() {
	...
	vector<string> fileToRemove = getListFileFrom("D:\\Test\\Characters\\");
	string fileToRemoveTmp;
	for each(string var in fileToRemove) {
		fileToRemoveTmp = "D:\\Test\\Characters\\" + var;
		const char* tmpChar = fileToRemoveTmp.c_str();
		remove(tmpChar);
	}
	fileToRemove.clear();
	fileToRemove = getListFileFrom("D:\\Test\\Training1\\");
	for each(string var in fileToRemove) {
		fileToRemoveTmp = "D:\\Test\\Characters\\Training1\\" + var;
		const char* tmpChar = fileToRemoveTmp.c_str();
		remove(tmpChar);
	}
	fileToRemove.clear();
	fileToRemove = getListFileFrom("D:\\Test\\Training2\\");
	for each(string var in fileToRemove) {
		fileToRemoveTmp = "D:\\Test\\Characters\\Training2\\" + var;
		const char* tmpChar = fileToRemoveTmp.c_str();
		remove(tmpChar);
	}
	...
	for (int i = 0; i < (vLabel.size() / 2) * 2; i++) {
		...
		if (limitSignT1 > nrSignT1[vLabel[i]]) {
			...
			tmpName = "D:\\Test\\Training1\\";
			tmpName += vNLP[i].getSign()[i];
			tmpName += "_" + itos(nrSign[numberSign]);
			tmpName += ".png";
			imwrite(tmpName, tmp[i]);
			nrT1Training++;
