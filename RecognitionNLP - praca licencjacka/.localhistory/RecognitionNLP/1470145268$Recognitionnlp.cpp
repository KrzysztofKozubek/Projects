...
void RecognitionNLP::test() {
	...
	for (int i = 0; i < vLabelTest.size(); i++) {
		labelTestI[i] = vLabelTest[i];
		dataTestI[i] = new float[vTrainingTest[i].size()];
		for (int j = 0; j < vTestData[i].size(); j++)
			dataTestI[i][j] = vTestData[i][j];
	}
}
