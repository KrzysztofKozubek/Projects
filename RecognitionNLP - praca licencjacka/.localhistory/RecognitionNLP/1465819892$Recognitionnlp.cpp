...
void RecognitionNLP::myTest() {
	...
	for (int i = 0; i < (vLabel.size() / 2) * 2; i++) {
		...
		if (limitSignT1 > nrSignT1[vLabel[i]]) {
			...
			help = "D:\\Segmentation\\Training1\\" + sign[vLabel[i]] + '_' + itos(i) + ".png";
		...
		if (limitSignT2 > nrSignT2[vLabel[i]]) {
			...
			help = "D:\\Segmentation\\Training2\\" + sign[vLabel[i]] + '_' + itos(i) + ".png";
			imwrite(help, allSign[i]);
			nrT2Training++;
			nrSignT2[vLabel[i]]++;
		}
			...
			if (i % 2) {
				...
				nrT2Zb1++;
				help = "D:\\Segmentation\\Test1\\" + sign[vLabel[i]] + '_' + itos(i) + ".png";
				imwrite(help, allSign[i]);
			}
			else {
				...
				nrT2Zb2++;
				help = "D:\\Segmentation\\Test2\\" + sign[vLabel[i]] + '_' + itos(i) + ".png";
				imwrite(help, allSign[i]);
			}
		}
