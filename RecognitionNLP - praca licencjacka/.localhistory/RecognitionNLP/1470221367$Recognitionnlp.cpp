...
#pragma endregion create parameters segmentation
	...
	//Linear svm-predict [options] test_file model_file output_file
	...
	for (int c = 1; c < 10000; c *= 2) {
		CSVM::cmdRun("echo " + PATH_LIBSVM + "svm-train.exe -s %svm% -t %kernel% -c " + itos(c) + " -q " + PATH_NVF_TRAINING + " " + PATH_MODEL_LINEAR + " >> " + GS_LINEAR);
		CSVM::cmdRun("echo echo Kernel = Linear, Test = I, C = " + itos(c) + ": Result classification = >> " + GS_LINEAR);
		CSVM::cmdRun("echo " + PATH_LIBSVM + "svm-predict.exe " + PATH_NVF_TEST_I + " " + PATH_MODEL_LINEAR + " >> " + GS_LINEAR);
		CSVM::cmdRun("echo echo Kernel = Linear, Test = II, C = " + itos(c) + ": Result classification = >> " + GS_LINEAR);
		CSVM::cmdRun("echo " + PATH_LIBSVM + "svm-predict.exe " + PATH_NVF_TEST_II + " " + PATH_MODEL_LINEAR + " >> " + GS_LINEAR);
	}
	...
	for (int c = 1; c < 10000; c *= 2) {
		for (double g = 0.001; g < 10; g *= 2) {
			CSVM::cmdRun("echo " + PATH_LIBSVM + "svm-train.exe -s %svm% -t %kernel% -c " + itos(c) + " -g " + itos(g) + " -q " + PATH_NVF_TRAINING + " " + PATH_MODEL_RBF + " >> " + GS_RBF);
			CSVM::cmdRun("echo echo Kernel = RBF, Test = I, C = " + itos(c) + ", g = " + itos(g) + ": Result classification = >> " + GS_RBF);
			CSVM::cmdRun("echo " + PATH_LIBSVM + "svm-predict.exe " + pathTest + test1Zb1 + " " + pathTest + test1Training + "02.model R.txt " + " >> " + GS_RBF);
			CSVM::cmdRun("echo echo Kernel = RBF, Test = II, C = " + itos(c) + ", g = " + itos(g) + ": Result classification = >> " + GS_RBF);
