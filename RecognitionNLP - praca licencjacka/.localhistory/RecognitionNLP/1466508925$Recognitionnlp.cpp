...
void RecognitionNLP::myTest() {
	...
	fileToRemove = getListFileFrom("C:\\Test\\zbiory\\uczacy\\proceswstepny\\");
	for each(string var in fileToRemove) {
		fileToRemoveTmp = "C:\\Test\\zbiory\\uczacy\\proceswstepny\\" + var;
		const char* tmpChar = fileToRemoveTmp.c_str();
		remove(tmpChar);
	}
	fileToRemove.clear();
	fileToRemove = getListFileFrom("C:\\Test\\zbiory\\segmentacja\\");
	for each(string var in fileToRemove) {
		fileToRemoveTmp = "C:\\Test\\zbiory\\uczacy\\segmentacja\\" + var;
		const char* tmpChar = fileToRemoveTmp.c_str();
		remove(tmpChar);
	}
	fileToRemove.clear();
...
void clearFolder(string path) {
	...
	string path = convertQStringToString(path);
	if ('\\' != path[path.length()])
		path += "\\";
	...
	vector<string> fileToRemove = getListFileFrom("C:\\Test\\zbiory\\uczacy\\deformacja\\");
	string fileToRemoveTmp;
	for each(string var in fileToRemove) {
		fileToRemoveTmp = "C:\\Test\\zbiory\\uczacy\\deformacja\\" + var;
		const char* tmpChar = fileToRemoveTmp.c_str();
		remove(tmpChar);
	}
	fileToRemove.clear();
