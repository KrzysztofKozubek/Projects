...
void RecognitionNLP::myTest() {
	...
	vector<CImageSign>	var;
	vector<string>		filesList = getListFileFrom(path);

	for each(string vNLP in filesList) {

		vNLP.push_back(CImageSign(imread(path + vNLP), vNLP));
		tmp.clear();
		tmp2 = var.getImage();
