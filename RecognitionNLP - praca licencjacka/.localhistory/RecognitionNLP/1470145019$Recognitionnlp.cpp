...
void RecognitionNLP::test() {
	...
	int*	labelTraining	= new int[vLabelTrain.size()];
	float** dataTraining	= new float*[vLabelTrain.size()];

	int*	labelTestI		= new int[vLabelTest.size()];
	float** dataTestI		= new float*[vLabelTest.size()];

	int*	labelTestII		= new int[vLabelTest.size()];
	float** dataTestII		= new float*[vLabelTest.size()];
	...
	/* PREAPEAR PARAMETERS NEED TO SEGMENTATION */
	...
	for (int i = 0; i < vLabelTrain.size(); i++) {
		...
		dataTraining[i] = new float[vTrainingTrain[i].size()];
		for (int j = 0; j < vTrainingTrain[i].size(); j++)
			dataTraining[i][j] = vTrainingTrain[i][j];
	}
