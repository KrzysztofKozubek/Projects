...
#pragma endregion create parameters segmentation
	...
	for (int c = 1; c < 10000; c *= 2) {
		for (double g = 0.001953125; g < 10; g *= 2) {
			...
			insertToFile(GS_RBF, "echo echo Kernel = RBF, Test = I, C = " + itos(c) + ", g = " + itos(g) + ": Result classification = ");
			insertToFile(GS_RBF, "echo ..\\" + PATH_LIBSVM + "svm-predict.exe ..\\" + PATH_NVF_TEST_I + " ..\\" + PATH_MODEL_RBF + " ..\\" + RESULT_FILE);
			insertToFile(GS_RBF, "echo echo Kernel = RBF, Test = II, C = " + itos(c) + ", g = " + itos(g) + ": Result classification = ");
	...
	for (int c = 1; c < 10000; c *= 2) {
		...
		insertToFile("TEST I.bat", "echo Linear, -c " + itos(c) + ": ");
		insertToFile("TEST I.bat", "echo ..\\" + PATH_LIBSVM + "svm-train.exe -s 0 -t 0 -c " + itos(c) + " -q ..\\" + PATH_NVF_TRAINING + " ..\\" + PATH_MODEL_LINEAR);
		for (double g = 0.001953125; g < 10; g *= 2) {
			insertToFile("TEST I.bat", "echo echo Kernel = RBF, Test = I, C = " + itos(c) + ", g = " + itos(g) + ": Result classification = ");
		}
	}
