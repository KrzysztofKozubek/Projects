...
void RecognitionNLP::test() {
	...
	const string RESULT_FILE	= PATH_LIBSVM + "Result.txt";

	/* Name need file to training and classification */
	const string VF_TRAINING	= PATH_CLASSIFIER + "VF_Training.txt";
	const string VF_TEST_I		= PATH_CLASSIFIER + "VF_Test_I.txt";
	const string VF_TEST_II		= PATH_CLASSIFIER + "VF_Test_II.txt";
	const string NVF_TRAINING	= PATH_CLASSIFIER + "NVF_Training.txt";
	const string NVF_TEST_I		= PATH_CLASSIFIER + "NVF_Test_I.txt";
	const string NVF_TEST_II	= PATH_CLASSIFIER + "NVF_Test_II.txt";

	/* MODEL FILE TO PREDICT */
	const string MODEL_LINEAR	= PATH_CLASSIFIER + "Model_Linear.model";
	const string MODEL_RBF		= PATH_CLASSIFIER + "Model_RBF";

	/* Name skripts to find best parameters (grid search) */
	const string GS_LINEAR		= "GS_Linear.bat";
	const string GS_RBF			= "GS_RBF.bat";
	...
	CDistorted distorted = CDistorted();
	...
	for each (string nameFile in fileList){
		...
		imageTMP = imread(PATH_SET + nameFile);
	...
	labelTestI		= new int[vLabelTest.size()];
	dataTestI		= new float*[vLabelTest.size()];

	labelTestII		= new int[vLabelTest.size()];
	dataTestII		= new float*[vLabelTest.size()];
	...
	fileList.clear();
	fileList = getListFileFrom(PATH_SET_TWO);	
	...
	CSVM nu_svm;
	/* SAVE TRAINING VF */
	nu_svm.saveVectorToFile(labelTraining, dataTraining, PATH_SCRIPTS + PATH_TRAINING, vLabelTrain.size());
	/* SAVE TEST I VF */
	nu_svm.saveVectorToFile(labelTestI, dataTestI, PATH_SCRIPTS + PATH_SET_ONE, vLabelTestI.size());
	/* SAVE TEST II VF */
	nu_svm.saveVectorToFile(labelTestII, dataTestII, PATH_SCRIPTS + PATH_SET_TWO, vLabelTestII.size());


	/* CREAETE GRID SEARCH */

	string GS_Linear	= PATH_SCRIPTS + GS_LINEAR;
	string GS_RBF		= PATH_SCRIPTS + GS_RBF;
	...
	for (int c = 1; c < 10000; c *= 2) {
		CSVM::cmdRun("echo " + PATH_LIBSVM + "svm-train.exe -s %svm% -t %kernel% -c " + itos(c) + " -q " + PATH_TRAINING + " " + PATH_SCRIPTS + PATH_TRAINING + " >> " + GS_Linear);
		CSVM::cmdRun("echo " + PATH_LIBSVM + "svm-predict.exe " + pathTest + test1Zb1 + " " + pathTest + test1Training + "00.model R.txt " + " >> " + GS_Linear);
		CSVM::cmdRun("echo " + PATH_LIBSVM + "svm-predict.exe " + pathTest + test1Zb2 + " " + pathTest + test1Training + "00.model R.txt " + " >> " + GS_Linear);
	}
	...
	for (int c = 1; c < 10000; c *= 2) {
		for (double g = 0.001; g < 10; g *= 2) {
			CSVM::cmdRun("echo " + PATH_LIBSVM + "svm-train.exe -s %svm% -t %kernel% -c " + itos(c) + " -g " + itos(g) + " -q " + pathTest + test1Training + " " + pathTest + test1Training + "02.model >> " + GS_RBF);
			CSVM::cmdRun("echo " + PATH_LIBSVM + "svm-predict.exe " + pathTest + test1Zb1 + " " + pathTest + test1Training + "02.model R.txt " + " >> " + GS_RBF);
			CSVM::cmdRun("echo " + PATH_LIBSVM + "svm-predict.exe " + pathTest + test1Zb2 + " " + pathTest + test1Training + "02.model R.txt " + " >> " + GS_RBF);
		}
	}
