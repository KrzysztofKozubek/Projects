...
public:
	...
	void saveVectorToFile(int* labels, float** trainingData, string pathFile, string nameFile, string pathLibsvm, int size, bool scale = true) {
		...
		if (scale) {
			string commend;
			commend = pathLibsvm + "svm-scale.exe " + pathFile + nameFile + " >> SCRIPTS\\" + nameFile;
			cmdRun(commend);
		}
	}
