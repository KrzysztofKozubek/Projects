...
#pragma endregion create parameters segmentation
	...
	for (int c = 1; c < 10000; c *= 2) {
		for (double g = 0.001953125; g < 10; g *= 2) {
			insertToFile(GS_RBF, "echo libsvm\\svm-train.exe -s %svm% -t %kernel% -c " + itos(c) + " -g " + itos(g) + " -q ..\\" + PATH_NVF_TRAINING + " ..\\" + PATH_MODEL_RBF);
			insertToFile(GS_RBF, "echo Kernel = RBF, Test = I, C = " + itos(c) + ", g = " + itos(g) + ":");
			insertToFile(GS_RBF, "echo libsvm\\svm-predict.exe ..\\" + PATH_NVF_TEST_I + " ..\\" + PATH_MODEL_RBF + " ..\\" + RESULT_FILE);
			insertToFile(GS_RBF, "echo Kernel = RBF, Test = II, C = " + itos(c) + ", g = " + itos(g) + ":");
			insertToFile(GS_RBF, "echo libsvm\\svm-predict.exe ..\\" + PATH_NVF_TEST_II + " ..\\" + PATH_MODEL_RBF + " ..\\" + RESULT_FILE);
		}
	}
