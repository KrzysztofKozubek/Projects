...
void RecognitionNLP::setParameters() {
	...
	for (double c = 100000; c > 0.0001; c/=2) {
		...
		CSVM::cmdRun("echo C:\\Test\\svm-train.exe -s %svm% -t %kernel% -c " + itos(c) + " -q C:\\Test\\SVF.txt C:\\Test\\SVFTraining.txt00.model >> C:\\Test\\linear.bat");
		CSVM::cmdRun("echo echo svm-train.exe -s %svm% -t %kernel% -c " + itos(c) + " \>\> C:\\Test\\r.txt >> C:\\Test\\linear.bat");
