...
void RecognitionNLP::myTest() {
		...
		for (int i = 0; i < tmp.size() && i < var.getSign().size() - 4; i++) {
			...
			if (numberSign != -1) {
				...
				if (limitSignT1 > nrSignT1[numberSign]) {
					...
					vTrainingTrain.push_back(*countMoment(tmp[i]));
					...
					Point2f sc;
					Mat rot_mat = getRotationMatrix2D((tmp[i].cols / 2.0F, tmp[i].rows / 2.0F), angle, 1.0);
					Mat dst;
					...
					help += "_" + itos(i) + ".png";
					imwrite(help, segmentation.rotation(tmp[i]));
					nrT1Training++;
				}
				else {

					vLabelTest.push_back(numberSign);
					vTrainingTest.push_back(*countMoment(tmp[i]));
	...
	for (int i = 0; i < vLabelTest.size(); i++) {
		labelT1Training[i] = vLabelTest[i];
	...
	//Linear
	CSVM::cmdRun("@echo OFF >> " + GS_Linear);
	CSVM::cmdRun("echo SET \"svm=0\" >> " + GS_Linear);
	CSVM::cmdRun("echo SET \"kernel=0\" >> " + GS_Linear);
	for (int c = 1; c < 10000; c *= 2) {
		CSVM::cmdRun("echo " + pathTest + "svm-train.exe -s %svm% -t %kernel% -c %" + itos(c) + "% -q " + pathTest + test1Training + " " + pathTest + test1Zb1 + "00.model >> " + GS_Linear);
		CSVM::cmdRun("echo " + pathTest + "svm-predict.exe " + pathTest + test1Zb1 + " " + pathTest + test1Training + " >> " + GS_Linear);
		CSVM::cmdRun("echo " + pathTest + "svm-predict.exe " + pathTest + test1Zb2 + " " + pathTest + test1Training + " >> " + GS_Linear);
	}

	//RBF
	CSVM::cmdRun("@echo OFF >> " + GS_RBF);
	CSVM::cmdRun("echo SET \"svm=0\" >> " + GS_RBF);
	CSVM::cmdRun("echo SET \"kernel=1\" >> " + GS_RBF);
	for (int c = 1; c < 10000; c *= 2) {
		for (double g = 0.001; g < 10; g *= 2) {
			CSVM::cmdRun("echo " + pathTest + "svm-train.exe -s %svm% -t %kernel% -c %" + itos(c) + "% -g %" + itos(g) + "% -q " + pathTest + test1Training + " " + pathTest + test1Zb2 + "00.model >> " + GS_RBF);
			CSVM::cmdRun("echo " + pathTest + "svm-predict.exe " + pathTest + test1Zb1 + " " + pathTest + test1Training + " >> " + GS_RBF);
			CSVM::cmdRun("echo " + pathTest + "svm-predict.exe " + pathTest + test1Zb2 + " " + pathTest + test1Training + " >> " + GS_RBF);
		}
	}
