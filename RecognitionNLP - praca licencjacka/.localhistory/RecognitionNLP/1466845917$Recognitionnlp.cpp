...
Mat RecognitionNLP::translateImg(Mat &img, int offsetx, int offsety) {
...
void RecognitionNLP::myTest() {
		...
		for (int i = 0; i < tmp.size() && i < var.size() - 4; i++) {
			...
			if (numberSign != -1) {
				...
				if (limitSignT1 > nrSignT1[numberSign]) {
					...
					int angle = (rand() % 31) - 15;
					int horizontal = rand() % 20 - 5;
					int vertical = rand() % 20 - 5;
					...
					dst = translateImg(dst, horizontal, vertical);
				...
				imshow("Aasd", translateImg(tmp[i], -10, 10));
				waitKey();
				imshow(itos(i), tmp[i]);
	...
	for (int c = 1; c < 10000; c *= 2) {
		...
		CSVM::cmdRun("echo " + pathTest + "svm-predict.exe " + pathTest + test1Zb1 + "00.model " + pathTest + test1Training + " >> " + GS_Linear);
		CSVM::cmdRun("echo " + pathTest + "svm-predict.exe " + pathTest + test1Zb1 + "00.model " + pathTest + test1Training + " >> " + GS_Linear);
	}
	...
	for (int c = 1; c < 10000; c *= 2) {
		for (double g = 0.001; g < 10; g *= 2) {
			CSVM::cmdRun("echo " + pathTest + "svm-train.exe -s %svm% -t %kernel% -c " + itos(c) + " -g " + itos(g) + " -q " + pathTest + test1Training + " " + pathTest + test1Zb2 + "02.model >> " + GS_RBF);
			CSVM::cmdRun("echo " + pathTest + "svm-predict.exe " + pathTest + test1Zb2 + "02.model " + pathTest + test1Training + " >> " + GS_RBF);
			CSVM::cmdRun("echo " + pathTest + "svm-predict.exe " + pathTest + test1Zb2 + "02.model " + pathTest + test1Training + " >> " + GS_RBF);
		}
	}
