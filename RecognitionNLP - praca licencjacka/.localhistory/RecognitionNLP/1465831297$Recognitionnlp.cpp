...
void RecognitionNLP::generatorSign() {
	...
	Mat tmpMat;
	string tmpPath = "";
	string path = "D:\\Segmentation\\Result\\";
	for (int i = 0; i < vNLP.size(); i++) {
		for (int j = 0; j < 100; j++) {

			tmpPath = path;
			tmpPath += vNLP[i].getSign();
			tmpPath += + "_" + itos(j) + ".png";
			tmpMat = (segmentation.segmentationNLP(vNLP, 0, 1, 0, 1))[0];
			imwrite(tmpPath, )
		}
	}
...
void RecognitionNLP::saveFileVectorFeauter() {
	...
	CSVM nu_svm;
	nu_svm.saveVectorToFile(label, trainingData, "libsvm\\VF.txt", vLabel.size(), false);
	nu_svm.~CSVM();
}
