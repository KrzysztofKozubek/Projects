...
//    number movies on a one site is 20
    ...
    public function getSearchMovies($search, $actor, $yearSince, $yearTo, $genre, $mark, $sort)
    {
        $query = "Call `p_search`('%$search%', '%$actor%', '$yearSince-01-01', '$yearTo-01-01', '%$genre[0]%', '%$genre[1]%', '$mark', '$sort')";
        $this->pdo->query('SET NAMES utf8');
        $select = $this->pdo->query($query);
        $data = null;
        foreach ($select as $row) {
