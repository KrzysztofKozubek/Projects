...
void OCR::saveEffect() {
	...
	for (int i = 0; i < vImageSignRow.size(); i++) {
		SegmentationCell(vImageRow[i]);
	}

	string fileName;
	for (int i = 0; i < vImageSignCell.size(); i++){

		fileName = ("digits//digit" + (itos(i % 101)) + "_" + (itos(i % 21)) + "_" + (itos(i % 10)) + ".jpg");
		imwrite(fileName, vImageSignCell[i]);
	}
}
