...
void OCR::GenerationSkryptLibsvm() {
	...
	for (int c = 1; c < 10000; c *= 2) {
		...
		CWindows::insertToFile(GS_LINEAR, "echo Kernel = Linear, Test = II, C = " + itos(c) + ":");
		CWindows::insertToFile(GS_LINEAR, "..\\" + PATH_LIBSVM + "svm-predict.exe ..\\" + PATH_NVF_TEST_II + " ..\\" + PATH_MODEL_LINEAR + " ..\\" + RESULT_FILE);
	}
