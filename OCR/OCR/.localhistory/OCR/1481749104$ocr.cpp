...
void OCR::SegmentationMaxSize(Mat& image) {
	...
	for (int y = 0; y < image.rows; y++) {
		sum = CPreProcessing::advRow(image, 0, 0);
		CPreProcessing::cutImage(image, left, image.rows - top, image.cols - right, 0);
		imshow("Z", image);
		waitKey();
		if (sum > 0) {
			bottom = y;
	...
	imshow("PRZED", image.clone());
	Mat tmp = image.clone();
	CPreProcessing::cutImage(tmp, left, image.rows - top, image.cols - right, 0);
	image = tmp.clone();
	imshow("po", image);
	waitKey();
}
	...
	for (int y = 0; y < cols - 1; y++) {
		...
		if (LIMIT_VALUE_LIGHT_BIT <= sum) {
			...
			if (cutStart == 1 && cutEnd == 1) {
				...
				Mat tmpp(Size(size * 2, size * 2), 0);
				for (int y = 0; y < size * 2; y++){
					for (int x = 0; x < size * 2; x++) {
						tmpp.at<uchar>(y, x) = 0;
						if (tmp.rows < y && tmp.cols < x) 
							tmpp.at<uchar>(y, x) = tmp.at<uchar>(y, x); 
					}
				}
				//CPreProcessing::getCenterWeightImage(tmpp);

				vImageSignCell.push_back(tmpp);
				tmp.release();
