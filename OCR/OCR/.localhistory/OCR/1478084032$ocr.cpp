...
OCR::OCR(QWidget *parent) : QMainWindow(parent) {
	...
	// 12 11 6
	this->imageSign		= imread("digit//digits08.jpg", 0);
...
void OCR::preProcessing() {
	...
	//Find value skip
	int skipT = CPreProcessing::getSkipValue(imageSign.clone());
	int skipL = 0;
	int skipR = 0;
	int skipB = 0;
	...
	CPreProcessing::toBinary(imageSign, ui.HSBinaryKernel3->value(), skipL, skipT, skipR, skipB);

	//Change angle
	CPreProcessing::autoRotate(imageSign, skip);

	CPreProcessing::cutImage(imageSign, 200, skip, 150, 0);
