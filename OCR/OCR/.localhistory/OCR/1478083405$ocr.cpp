...
OCR::OCR(QWidget *parent) : QMainWindow(parent) {
	...
	// 12 11 6
	this->imageSign		= imread("digit//digits06.jpg", 0);
...
void OCR::preProcessing() {
	...
	//Find value skip
	int skip = CPreProcessing::getSkipValue(imageSign.clone());

	CPreProcessing::dilate(imageSign, 2, 1);

	//Binary
	//threshold(tmp, imageSign, ui.HSBinaryKernel3->value(), 255, ui.HSBinaryElement3->value());
	CPreProcessing::toBinary(imageSign, ui.HSBinaryKernel3->value(), skip);

	//Change angle
	CPreProcessing::autoRotate(imageSign, skip);

	CPreProcessing::cutImage(imageSign, 150, skip, 150, 100);
