...
public:
	...
	static int advM(Mat image, int row, int col, int size) {
	...
	static int blackOrWhite(Mat image, int y, int x) {

		int margin = 200;
		if (image.at<uchar>(y, x) < advM(image, y, x, 2))	return 255;
		else												return 0;
		return 125;
	}
	...
	static void toBinary(Mat& image, int margin, int skipL = 50, int skipT = 50, int skipR = 50, int skipB = 50) {
		...
		threshold(image.clone(), image, margin, 255, THRESH_BINARY_INV | ADAPTIVE_THRESH_MEAN_C);
		threshold(image.clone(), image, margin, 255, THRESH_BINARY);
		...
		for (int y = skipT; y < rows - skipB; y++) {
			for (int x = skipL; x < cols - skipR; x++) {
				//image.at<uchar>(y, x) = blackOrWhite(tmp, y, x);
			}
		}
	}
