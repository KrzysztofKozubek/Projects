...
public:
	...
	static void getCenterWeightImage(Mat& image) {
		...
		Mat affine_matrix = Mat::eye(2, 3, 0);
		*(((float*)(affine_matrix.data)) + 2) = getXCenterWeight(image);
		*(((float*)(affine_matrix.data)) + 5) = getYCenterWeight(image);
		warpAffine(image.clone(), image, affine_matrix, image.clone().size());
	}
