...
void OCR::SegmentationMaxSize(Mat& image) {
	...
	for (int y = image.rows / 3; y < image.rows /3*2; y+=2) {
	...
	for (int y = image.rows / 3 * 2; y >= image.rows / 3; y -= 2) {
		for (int x = 0; x < image.cols; x++) {
			if (image.at<uchar>(y, x) != 0) {
				top = y;
				y = 0;
	...
	for (int x = image.cols / 3; x < image.cols / 3*2; x += 2) {
	...
	for (int x = image.cols /3*2; x >= image.cols / 3; x-=2) {
