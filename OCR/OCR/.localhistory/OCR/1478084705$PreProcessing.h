...
public:
	...
	static int getSkipRValue(Mat image) {
		...
		while (skip < image.rows / 4) {

			for (int i = image.rows / 2 - margin; i < image.rows / 2 + margin; i++) {
				if (image.at<uchar>((int)(image.rows / 2), i) > 0) result++;
			}
		}
			
			&& ((image.at<uchar>(skip, center[0]) <= 0) || (image.at<uchar>(skip, center[1]) <= 0) || (image.at<uchar>(skip, center[2]) <= 0))) skip++;
