...
public:
	...
	static void removeBlackBackground(Mat& image) {
		...
		//up
		int up = 0;
		while (margin > advRow(image, up, 1) && image.rows > up) up++;

		//bottom
		int bottom = 0;
		while (margin > advRow(image, image.rows - bottom - 1, 1) && image.rows > up + bottom) bottom++;

		cutImage(image, 0, up, 0, bottom);

		//right
		int right = 0;
		while (margin > advCol(image, image.cols - right - 1, 1) && image.cols > right) right++;
		
		/*{

			cutImage(image, 0, 0, 1, 0);
			imshow("Z", image);
			waitKey();
		}*/
		...
		//left
		int left = 0;
		while (margin > advCol(image, left, 1) && image.cols > left + right) left++;
