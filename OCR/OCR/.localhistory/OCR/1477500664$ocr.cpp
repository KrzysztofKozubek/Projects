...
OCR::OCR(QWidget *parent) : QMainWindow(parent) {
	...
	this->imageSign = imread("digits04.jpg", CV_BGR2GRAY);
	this->imageSignO = imageSign.clone();
	autorotate(imageSign);
	//changeBinaryOrErode();
...
void OCR::autorotate(Mat iamge) {
	...
	int erosion_size2 = 2;
	Mat element2 = getStructuringElement(MORPH_CROSS, Size(2 * erosion_size2 + 1, 2 * erosion_size2 + 1), Point(erosion_size2, erosion_size2));
	dilate(imageSignO.clone(), tmp, element2);

	for (int y = 0; y < tmp.rows; y++) {
		for (int x = 0; x < tmp.cols; x++) {
			if (tmp.at<uchar>(y, x) > 50)
				tmp.at<uchar>(y, x) = 0;
			else tmp.at<uchar>(y, x) = 255;
		}
	}

	int y = 0; int x = tmp.cols / 2;
	while (y < image.rows && image.at<uchar>(y, x) > 0) { y++; }

	double begin = -0.5, end = 0.5, precision = 0.1;
	int sum = sumRow(tmp, y, 1), v = begin, tmpSum = 0;

	cutImage(tmp, 300, 0, 0, 0);
	imshow("S", tmp);
	waitKey();

	for (double a = begin; a <= end; a += precision) {

		Point2f src_center(tmp.cols / 2.0F, tmp.rows / 2.0F);
		Mat rot_mat = getRotationMatrix2D(src_center, a, 1.0);
		warpAffine(tmp, showImage, rot_mat, showImage.size());

		tmpSum = sumRow(showImage, y, 1);

		myfile << a << "=" << advRow(showImage, y, 1) << "\n";
		if (tmpSum > sum) {
			sum = tmpSum;
			v = a;
			imshow("S", showImage);
...
int change(Mat image, int row, int col, int value) {

	for (int y = row - 1; y < row + 1 && y < image.rows; y++) {
		for (int x = col - 1; x < col + 1 && x < image.cols; x++) {

			if (y != row && x != col && y > 0 && x > 0) {
				if (value < image.at<uchar>(y, x)) {
					return 0;
				}
			}
		}
	...
	for (int x = 0; x < tmp.cols; x++) {
		for (int y = 0; y < tmp.rows; y++) {
			...
			if (tmp.at<uchar>(y, x) > ui.HSBinaryKernel3->value())
				imageSign.at<uchar>(y, x) = change(tmp, y, x, ui.HSBinaryKernel3->value());
			else
				imageSign.at<uchar>(y, x) = change(tmp, y, x, ui.HSBinaryKernel3->value());
		}
	}
