...
public slots:
	...
	int sumColumn(Mat image, int col, int NUMBER_MARGIN_MISSTAKE);
	int sumRow(Mat image, int row, int NUMBER_MARGIN_MISSTAKE);
	double advRow(Mat image, int row, int NUMBER_MARGIN_MISSTAKE);
	double advCol(Mat image, int col, int NUMBER_MARGIN_MISSTAKE);
	Mat removeBlackBackground(Mat image);
	void rotate();
	void confirmChange();
	void loadImage();
	void saveImage();
	...
	string createNameFile(int digit, int user, int sample);
