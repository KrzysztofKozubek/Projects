...
void displayHist(vector<double> values) {

	double* histSample = new double[values.size()];
	histSample[0] = new double[values.size()];
	for (int i = 0; i < values.size(); i++) {
		histSample[0][i] = values[i];
	}

	
	Mat histogram(255, 255, CV_32F, (void*)histSample); //OR CV_64F
	int histSize = 255;
