...
public:
	...
	static int advM(Mat image, int row, int col, int size) {
		...
		int sum		= 0;
		int sSum	= 0;
		for (int x = col - size; x < col + size && x < image.cols && x >=0 ; x++) {
			for (int y = row - size; y < row + size && x < image.rows && y >= 0; y++) {
				sum += image.at<uchar>(y, x);
				sSum++;
			}
		}
	...
	static int blackOrWhite(Mat image, int y, int x) {
		...
		int margin = 200;
		if (image.at<uchar>(y, x) < advM(image, y, x, 5)) return 255;
		else								return 0;
		return 1;
	}
	...
	static void toBinary(Mat& image, int margin, int skipL = 50, int skipT = 50, int skipR = 50, int skipB = 50) {
		...
		for (int y = skipT; y < rows - skipB; y++) {
			for (int x = skipL; x < cols - skipR; x++) {
				image.at<uchar>(y, x) = blackOrWhite(image.clone(), y, x);
			}
		}
	}
