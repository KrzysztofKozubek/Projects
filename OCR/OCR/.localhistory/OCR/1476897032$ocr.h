...
public slots:
	...
	void cutImage(Mat & image, const int cutLeft, const int cutTop, const int cutRight, const int cutBottom);
	int sumColumn(Mat image, int row, int NUMBER_MARGIN_MISSTAKE);
	int sumRow(Mat image, int col, int NUMBER_MARGIN_MISSTAKE);
	double advRow(Mat image, int row, int NUMBER_MARGIN_MISSTAKE);
	double advCol(Mat image, int col, int NUMBER_MARGIN_MISSTAKE);
	void rotate();
	void confirmChange();
	void loadImage();
	void saveImage();

	void SegmentationDisplayImage(Mat image);
	void SegmentationLoadImage();
	void SegmentationSaveImage();

	void SegmentationCutImage(Mat& image, int x1, int y1, int x2, int y2);
