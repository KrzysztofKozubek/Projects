...
int val(Mat image, int row, int col, int sizeM) {

	int n = 0;
	for (int y = row - sizeM; y < row + sizeM && y < image.rows; y++) {
		for (int x = col - sizeM; x < col + sizeM && x < image.cols; x++) {

			if (y != row && x != col && y > 0 && x > 0) {
				if (0 != image.at<uchar>(y, x)) {
					n++;
				}
			}
		}
	}
	return n > sizeM / 2 ? 255 : 0;
}
	...
	for (int x = 0; x < tmp.cols; x++) {
		for (int y = 0; y < tmp.rows; y++) {
			...
			if (tmp.at<uchar>(y, x) > ui.HSBinaryKernel3->value())
				imageSign.at<uchar>(y, x) = val(image, y, x, 1);
			else
