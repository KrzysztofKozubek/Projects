...
OCR::OCR(QWidget *parent) : QMainWindow(parent) {
	...
	string a = "ASD_8.jpg";
	int n = atoi(a.substr(a.size()-5,a.size()-4).c_str());
	cout << n;
...
void OCR::MomentsLoadImage() {
	...
	for (int i = 0; i < vImageSign.size(); i++) label[i] = atoi(vImage[i].erase(-5, -4).c_str());

	//stworz tablice float**
	//stworz tablice label*
