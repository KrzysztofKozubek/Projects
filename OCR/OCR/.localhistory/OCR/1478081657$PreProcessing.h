...
public:
	...
	static int getSkipValue(Mat image) {

		int skip = 0;
		CPreProcessing::dilate(image, 2, 2);
		while (skip < image.rows && iamge.at<uchar>(y, image.cols / 2) < 1 && iamge.at<uchar>(y, (image.cols / 2) - 1) < 1 && iamge.at<uchar>(y, (image.cols / 2) + 1) < 1) skip++;
		for (int y = 0; y < y < image.rows&&image.at<uchar>(y, iamge.cols / 2); y++)
		return 1;
	}
