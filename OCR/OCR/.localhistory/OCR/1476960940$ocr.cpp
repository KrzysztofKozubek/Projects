...
Mat OCR::removeBlackBackground(Mat image) {
	...
	//up
	int up = 0;
	while (margin > sumRow(image, 0, 1) && image.rows < up);

	//right
	int right = 0;
	while (margin > sumColumn(image, image.cols - right - 1, 1) && image.cols < right);

	//bottom
	int bottom = 0;
	while (margin > sumRow(image, image.rows - bottom - 1, 1));

	//left
	int left = 0;
	while (margin > sumColumn(image, left, 1) && image.cols < left + right);

	Mat tmp = image.clone();
	cutImage(tmp, left, up, right, bottom && image.rows < up + bottom);
