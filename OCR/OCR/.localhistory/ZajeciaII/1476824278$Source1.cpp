...
int SegmentationSumRow(Mat image, int row, int NUMBER_COLUMN_GET_TO_SEGMENTATION) {
...
double SegmentationAverageRow(Mat image, int row, int NUMBER_COLUMN_GET_TO_SEGMENTATION) {

	return SegmentationSumRow(image, row, NUMBER_COLUMN_GET_TO_SEGMENTATION) / NUMBER_COLUMN_GET_TO_SEGMENTATION;
}
...
int SegmentationSumCol(Mat image, int col, int NUMBER_COLUMN_GET_TO_SEGMENTATION) {

	double sum = 0;
	for (int x = col; x < image.cols && x < image.cols + NUMBER_COLUMN_GET_TO_SEGMENTATION; x++) {
		for (int y = 0; y < image.rows; y++) {

			if (image.at<uchar>(y, x) != 0) sum++;
		}
	}
...
double SegmentationAgerageCol(Mat image, int col, int NUMBER_COLUMN_GET_TO_SEGMENTATION) {

	return SegmentationSumCol(image, col, NUMBER_COLUMN_GET_TO_SEGMENTATION) / NUMBER_COLUMN_GET_TO_SEGMENTATION;
}
...
void SegmentationCell(int, void*) {
	...
	for (int y = 0; y < cols - 1; y++) {
		...
		sum = SegmentationAgerageCol(image, y, 3);
		if (y == 240) {
			waitKey();
		}
...
int main() {
	...
	erode(im, image, element);

	LIMIT_VALUE_LIGHT_BIT = 245;
	createTrackbar("Value", "0", &LIMIT_VALUE_LIGHT_BIT, maxA, SegmentationCell);
	imshow("0", (image));
	SegmentationCell(LIMIT_VALUE_LIGHT_BIT, 0);
	waitKey(0);
}