	...
	for (int y = 0; y < cols - 1; y++) {
		...
		if (LIMIT_VALUE_LIGHT_BIT >= sum) {
			...
			if (cutStart == 1 && cutEnd == 1) {
				...
				if (pomoc == 1)
					imshow("1", normalaizedImage(tmp.clone()));
				if (pomoc == 2)
					imshow("2", normalaizedImage(tmp.clone()));
				if (pomoc == 3)
					imshow("3", normalaizedImage(tmp.clone()));
				if (pomoc == 4)
					imshow("4", normalaizedImage(tmp.clone()));
