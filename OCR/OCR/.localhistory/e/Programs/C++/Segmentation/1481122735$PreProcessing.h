...
#pragma region static methods
	...
	static void changeContrasOrBrightness(cv::Mat& image, int alpha, int beta) {
		...
		for (int y = 0; y < image.rows; y++) {
			for (int x = 0; x < image.cols; x++) {
				for (int c = 0; c < 3; c++) {
					image.at<Vec3b>(y, x)[c] = cv::saturate_cast<uchar>(alpha*(image.(cv::at<Vec3b>(y, x)[c])) + beta);
				}
			}
		}
	}
	...
	static void dilate(cv::Mat& image, int type, int size) {
		...
		cv::Mat element = getStructuringElement(type, Size(2 * size + 1, 2 * size + 1), Point(size, size));
	...
	static void getCenterWeightImage(cv::Mat& image) {
		...
		cv::Mat trans_mat = (Mat_<double>(2, 3) << 1, 0, x, 0, 1, y);
