...
public:
	...
	string result() {
		...
		string result, tmp;
		CWindows cmd;
		cmd.cmdRun("del libsvm\\NF.txt libsvm\\SNF.txt libsvm\\result.txt");
		...
		cmd.cmdRun("libsvm\\svm-predict.exe libsvm\\SNF.txt libsvm\\NVF_Training.txt >> result.txt");

		result = CWindows::readFile("libsvm\\result.txt");
		return result;
	}
