...
public:
	...
	void MaxSizeB(Mat& image, int margin = 1) {
		...
		for (int y = 0; y < tmp.rows; y++) {
			sum = CPreProcessing::sumRow(tmp, y, 1);
			if (sum > margin) {
				top = y;
				y = image.rows;
			}
	...
	vector<Mat> findSignB(Mat image) {
		...
		for (int y = 0; y < cols - 1; y++) {
			sum = CPreProcessing::sumColumn(image, y, 2);
			//cout << " sum:" << sum << " " << endl;
