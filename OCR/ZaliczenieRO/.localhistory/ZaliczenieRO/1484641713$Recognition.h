...
public:
	...
	static string recognition(Mat image) {

		string result = "Kozubek\\result.txt";
		int*	label = new int[1];
		float** data = new float*[1];

		for (int i = 0; i < 1; i++)		label[i]	= 0;
		for (int i = 0; i < 1; i++)		data[i]		= countMoment(image);
		CWindows::saveVectorToFile(label, data, "Kozubek\\VF_Test.txt", 1, 0);
		CWindows::saveVectorToFile(label, data, "Kozubek\\NVF_Test.txt", 1, 1);


		//CWindows::cmdRun("Kozubek\\libsvm\\predict.exe Kozubek\\NVF_Test.txt Kozubek\\libsvm\\model.model " + result);
