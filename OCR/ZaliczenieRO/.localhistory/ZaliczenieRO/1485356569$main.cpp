	...
	for each(string var in images) {
		...
		if (var[0] == 'a') {
			...
			for each(Mat varr in tmp) {
				...
				//CWindows::cmdRun("echo " + var + " >> " + var + ".txt");
			}
		}
		else {
			if (var[0] == 'b') {
		...
		//var.substr(0, var.length()-4) + " " + 
		CWindows::cmdRun("echo " + CRecognition::recognition(result) + " >> " + nameFileResult);
	}
}