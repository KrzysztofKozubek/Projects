...
public:
	...
	static void saveVectorToFile(int* labels, float** trainingData, string nameFile, int size, bool scale = true) {
		...
		string pathScale = "Kozubek\\libsvm\\scale.exe";
		string pathScaleModel = "Kozubek\\libsvm\\scale";
		string pathNVFTest = "Kozubek\\NVF_Test.txt";
		string tmp = "Kozubek\\libsvm\\tmp.txt";
		if (!scale)
			tmp = nameFile;
		...
		if (scale) {
			//scaling parameters
			commend = "Kozubek\\libsvm\\scale.exe -r Kozubek\\libsvm\\scale " + tmp + " >> Kozubek\\NVF_Test.txt";
			cmdRun(commend);
		}
	}
