...
public:
	...
	void createFileOfMoments() {
		...
		int	label[] = { 3, 4, 3, 5, 2 };//new int[images.size()];
		float** data = new float*[images.size()];

		for (int i = 0; i < images.size(); i++) data[i] = countMoment(images[i]);
		//for (int i = 0; i < images.size(); i++) label[i] = atoi(images[i].substr(images[i].size() - 5, images[i].size() - 4).c_str());
...
private:
	...
	void GenerationSkryptLibsvm(vector<Mat> images) {
