...
import models.{ UserRegistered, User }
...
class SocialAuthController @Inject() (
  ...
  cache: CacheApi,
  userRegisteredDAO: UserRegisteredDAO
)
        ...
        case Some(p: SocialProvider with CommonSocialProfileBuilder) =>
          p.authenticate().flatMap {
            ...
            case Right(authInfo) => for {
              ...
              var registeredUser: UserRegistered = new UserRegistered(id = 0, idUser = user.loginInfo.providerKey, provider = user.loginInfo.providerID,
              ...
              userRegisteredDAO.insert(registeredUser)
              silhouette.env.eventBus.publish(LoginEvent(user, request))
              var strUser = Json.obj("user" -> user) + ""
              //Ok(Json.obj("user" -> user)).withSession("user" -> strUser)
              Redirect("http://localhost:4200/?auth=" + registeredUser.idUser, 302).withSession("user" -> strUser)
            }
          }
  ...
  def getLoggedUser = Action.async { implicit request =>
    ...
    request.session.get("user").map { user =>
      {
        val idUser1 = user.split("providerKey\":\"").last
        val idUser2 = idUser1.split("\"},\"firstName").head
        idUser = idUser2
      }
    }
